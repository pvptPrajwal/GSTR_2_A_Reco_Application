...
def detect_month_from_filename(filepath: str) -> str | None:
    """
    Try to detect month label from filename.
    Examples that work:
      April_GST.xlsx       → 'April'
      gst_may_2025.xlsx    → 'May'
      PB_06_2025.xlsx      → 'June'
      purchase_03.xlsx     → 'March'
    Returns None if month can't be detected.
    """
    name = Path(filepath).stem.lower()

    # Check full/abbreviated month names
    for i, (full, abbr) in enumerate(zip([m.lower() for m in MONTHS], MONTH_ABBR)):
        if full in name or abbr in name:
            return MONTHS[i]

    # Check numeric month (01–12)
...
def detect_month_from_data(df: pd.DataFrame) -> str | None:
    """
    If filename gives no clue, look at Invoice Date column.
    Take the most common month in the data.
    """
...
def _rename_cols(df):
    """Apply COL_ALIASES to normalise column names."""
...
def _extract_sheet(filepath, sheet_name):
    """Extract standard columns from one sheet. Returns None if no usable data."""
    ...
    chunk["Vendor Name"]     = _get_str_col(raw, "Vendor Name")   # Trade/Legal name from GST portal
...
def load_gst_file(filepath: str, log) -> pd.DataFrame:
    """Load one GST Portal file — scan all sheets, stack usable ones."""
...
def load_pb_file(filepath: str, log) -> pd.DataFrame:
    """Load one Purchase Book file — first sheet."""
    xl    = pd.ExcelFile(filepath)
    ...
    if "Vendor Name"  not in raw.columns: raw["Vendor Name"]  = ""   # Party Name from PB
    ...
    raw = raw[raw["Invoice Number"] != ""]
    keep = KEY_COLS + ["Vendor Name", "Invoice Date"] + VALUE_COLS
    raw  = raw[[c for c in keep if c in raw.columns]].copy()
    log(f"    {len(raw):,} rows from {Path(filepath).name}")
    return raw.reset_index(drop=True)


# ─────────────────────────────────────────────────────────────────────────────
# SECTION 4 — GROUP FILES BY MONTH
# ─────────────────────────────────────────────────────────────────────────────

def group_files_by_month(filepaths: list[str], file_type: str, log) -> dict[str, list[str]]:
    """
    Groups a list of file paths by detected month label.
    Returns dict: { 'April': [path1, path2], 'May': [path3], ... }
    Files where month can't be detected go into 'Unknown'.
    """
...
#     """
#     Remove exact duplicate rows (same GSTIN + Invoice No + all 4 values).
#     Returns (cleaned_df, duplicate_df).
#     """
...
#         for inv in dup_df["Invoice Number"].unique()[:5]:
#             log(f"       → {inv}: {is_dup[df['Invoice Number']==inv].sum()} extra copy/copies")
#     else:
...
def aggregate(df: pd.DataFrame, label: str, log) -> pd.DataFrame:
    ...
    # Carry Vendor Name — take first non-empty value per group
...
def reconcile(pb: pd.DataFrame, gst: pd.DataFrame, log):
    """Outer merge → 4 buckets: matched / mismatched / miss_gst / miss_pb
    Vendor Name from PB  → 'Vendor Name (PB)'
    Vendor Name from GST → 'Vendor Name (GST)'
    """
    # Rename Vendor Name before merge to avoid collision
    pb_m  = pb.copy()
    gst_m = gst.copy()
    ...
    # Add placeholder vendor name cols if missing
    ...
    # Mismatch if ANY single column (Taxable Value, IGST, CGST, SGST)
    # differs by more than the tolerance — checked individually so that
    # opposite-sign differences in different columns don't cancel out.
    diff_cols = [f"{c} Diff" for c in VALUE_COLS]
    both["Total Tax Diff"] = both[diff_cols].abs().sum(axis=1).round(2)
    is_mismatch = (both[diff_cols].abs() > MISMATCH_TOL).any(axis=1)

    matched    = both[~is_mismatch].copy()
    mismatched = both[is_mismatch].copy()
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 9B — VENDOR-WISE SUMMARY BUILDER
# ─────────────────────────────────────────────────────────────────────────────

def build_vendor_summary(pb_agg: pd.DataFrame, gst_agg: pd.DataFrame) -> pd.DataFrame:
    """
    Builds a vendor-wise comparison table for one month.
    Groups by GSTIN and sums all value columns from PB and GST separately,
    then calculates the difference.

    Output columns:
      GSTIN | Vendor Name (PB) | Vendor Name (GST)
      TV_PB | IGST_PB | CGST_PB | SGST_PB
      TV_GST| IGST_GST| CGST_GST| SGST_GST
      Diff_TV | Diff_IGST | Diff_CGST | Diff_SGST
    """
    # Aggregate PB by GSTIN
...
def _write_vendor_summary_sheet(ws, vendor_df: pd.DataFrame, month: str):
    """
    Write the vendor-wise summary table into worksheet ws.
    Two header sections:
      - Title row
      - Column headers with colour-coded groups (PB = blue, GST = green, Diff = orange)
    """
    # Title
    ...
    fills = {
        "key"  : PatternFill("solid",start_color="1F4E79",end_color="1F4E79"),  # GSTIN
        "pb"   : PatternFill("solid",start_color="1F5C2E",end_color="1F5C2E"),  # PB cols
        "gst"  : PatternFill("solid",start_color="7030A0",end_color="7030A0"),  # GST cols
        "diff" : PatternFill("solid",start_color="C55A11",end_color="C55A11"),  # Diff cols
    }
    ...
    for r, (_, row) in enumerate(vendor_df.iterrows(), 5):
        for c, h in enumerate(headers, 2):
            ...
            if c in money_cols:
                ...
                # Highlight non-zero differences in red
...
def build_cross_match(miss_gst: pd.DataFrame, miss_pb: pd.DataFrame) -> pd.DataFrame:
    """
    Find GSTINs that appear in BOTH Missing_in_GST_Portal AND
    Missing_in_Purchase_Book sheets.

    For those GSTINs, sum up values from each side and show the diff.
    ...
    # Find GSTINs common to both sheets
    ...
    # Filter both sides to common GSTINs only
    mg = miss_gst[miss_gst["GSTIN"].isin(common_gstins)].copy()
    mp = miss_pb [miss_pb ["GSTIN"].isin(common_gstins)].copy()

    # Sum value columns per GSTIN on PB side
    pb_grp = mg.groupby("GSTIN", as_index=False)[VALUE_COLS].sum().round(2)
    pb_grp.rename(columns={
        "Taxable Value": "TV_PB",
        "IGST": "IGST_PB", "CGST": "CGST_PB", "SGST": "SGST_PB"
    }, inplace=True)

    # Sum value columns per GSTIN on GST side
    gst_grp = mp.groupby("GSTIN", as_index=False)[VALUE_COLS].sum().round(2)
    gst_grp.rename(columns={
        "Taxable Value": "TV_GST",
        "IGST": "IGST_GST", "CGST": "CGST_GST", "SGST": "SGST_GST"
    }, inplace=True)

    # Pick vendor names — PB side first, GST side fallback
    def _first_name(df, col):
        if col not in df.columns:
            return pd.Series([""] * len(df["GSTIN"].unique()),
                             index=df.groupby("GSTIN").first().index)
        return df.groupby("GSTIN")[col].first()

    vn_pb  = _first_name(mg, "Vendor Name (PB)") \
             if "Vendor Name (PB)"  in mg.columns \
             else _first_name(mg, "Vendor Name")
    vn_gst = _first_name(mp, "Vendor Name (GST)") \
             if "Vendor Name (GST)" in mp.columns \
             else _first_name(mp, "Vendor Name")
    ...
    # Merge PB + GST totals
    result = vn_df.merge(pb_grp,  on="GSTIN", how="left") \
                  .merge(gst_grp, on="GSTIN", how="left")

    # Fill any NaN with 0
    for col in ["TV_PB","IGST_PB","CGST_PB","SGST_PB",
                "TV_GST","IGST_GST","CGST_GST","SGST_GST"]:
        result[col] = result[col].fillna(0.0)

    # Calculate differences (PB − GST)
...
def _write_cross_match_sheet(ws, df: pd.DataFrame):
    """Write the Cross_Match sheet with colour-coded headers."""
    # Title
    ...
    hfills = {
        "pb"  : PatternFill("solid", start_color="C00000", end_color="C00000"),  # dark red = miss_gst side
        "gst" : PatternFill("solid", start_color="7030A0", end_color="7030A0"),  # purple = miss_pb side
        "diff": PatternFill("solid", start_color="C55A11", end_color="C55A11"),  # orange = diff
        "key" : PatternFill("solid", start_color="1F4E79", end_color="1F4E79"),  # navy = GSTIN
    }
...
def export_report(matched, mismatched, miss_gst, miss_pb,
    ...
    """
    Flat single-reconciliation export.
    Sheets:
      Summary  |  Matched  |  Mismatched  |
      Missing_in_GST_Portal  |  Missing_in_Purchase_Book  |
      Vendor_Summary  |  Duplicates_Removed (if any)
    """
    ...
    # Cross_Match always added (shows "no common GSTINs" msg if empty)
    data_sheets.append(("Cross_Match", cross_df, "cross"))
    # if not all_dups.empty:   # DUPLICATES DISABLED
    #     data_sheets.append(("Duplicates_Removed", all_dups, "Duplicates_Removed"))

    # ── Write data sheets (skip vendor + cross — written manually via openpyxl) 
    with pd.ExcelWriter(str(out_path), engine="openpyxl") as writer:
        for name, df, colour_key in data_sheets:
            if colour_key not in ("vendor", "cross"):
...
def _build_combined_vendor_table(month_results: dict) -> pd.DataFrame:
    """
    Combine vendor-wise data across ALL months into one table.
    Groups by GSTIN, sums PB and GST values, calculates total diffs.
    Also picks best available Vendor Name (GST Portal first, PB fallback).
    """
    all_pb  = []
    all_gst = []

    for month, result_tuple in month_results.items():
        pb_agg  = result_tuple[4] if len(result_tuple) >= 6 else pd.DataFrame()
        gst_agg = result_tuple[5] if len(result_tuple) >= 6 else pd.DataFrame()
        if not pb_agg.empty:
            all_pb.append(pb_agg)
        if not gst_agg.empty:
            all_gst.append(gst_agg)

    combined_pb  = pd.concat(all_pb,  ignore_index=True) if all_pb  else pd.DataFrame()
    combined_gst = pd.concat(all_gst, ignore_index=True) if all_gst else pd.DataFrame()

    return build_vendor_summary(combined_pb, combined_gst)


def _write_summary(ws, summary_data: list[dict], all_dups: pd.DataFrame,
                   month_results: dict = None):
    """Write the overall summary dashboard."""
    # Title
    ws["B2"]      = "GSTR-2A vs Purchase Book — Multi-Month Reconciliation Report"
    ...
    # Table headers
    headers = ["Month","Matched","Mismatched","Miss. in GST",
               "Miss. in PB","Taxable Value (PB)","Total Tax (PB)","ITC At Risk"]
    ...
    row_fills = [
        PatternFill("solid",start_color="D9EAD3",end_color="D9EAD3"),
        PatternFill("solid",start_color="EBF3FB",end_color="EBF3FB"),
    ]

    for r_off, row in enumerate(summary_data, 6):
        fill = row_fills[r_off % 2]
        vals = [
            row["Month"], row["Matched"], row["Mismatched"],
            row["Miss_GST"], row["Miss_PB"],
            row["Taxable_PB"], row["Total_Tax_PB"], row["ITC_At_Risk"],
        ]
        for c,v in enumerate(vals,2):
            cell = ws.cell(r_off,c,v)
            cell.font=Font(name="Arial",size=10); cell.fill=fill; cell.border=THIN
            is_money = isinstance(v,float)
            is_risk  = (c == 9)   # ITC At Risk column
            cell.alignment=Alignment(
                horizontal="right" if isinstance(v,(int,float)) else "left",
                vertical="center")
            if is_money: cell.number_format="#,##0.00"
            if is_risk and isinstance(v,float) and v>0:
                cell.font=Font(name="Arial",size=10,bold=True,color="C00000")

    # Totals row
    last = 5 + len(summary_data)
    tr   = last + 1
    ws.cell(tr,2).value="TOTAL"
    ws.cell(tr,2).font=Font(name="Arial",bold=True,size=10)
    ws.cell(tr,2).fill=YELLOW_FILL
    for ci in range(3,10):
        cl = get_column_letter(ci); tc=ws.cell(tr,ci)
        tc.value=f"=SUM({cl}6:{cl}{last})"
        tc.font=Font(name="Arial",bold=True,size=10)
        tc.fill=YELLOW_FILL; tc.border=THIN
        tc.number_format="#,##0.00"
        tc.alignment=Alignment(horizontal="right")
...
#         note_row = tr + 2
#         ws.cell(note_row,2).value = f"🗑️  Total Duplicate Rows Removed: {len(all_dups)}  (see 'Duplicates_Removed' sheet)"
#         ws.cell(note_row,2).font  = Font(name="Arial",bold=True,size=10,color="595959")

    # ── VENDOR-WISE COMBINED TABLE (all months) ───────────────────────────────
    if month_results:
        vendor_df  = _build_combined_vendor_table(month_results)
        vendor_row = tr + 5          # leave a gap below the monthly summary

        # Section title
        ws.cell(vendor_row, 2).value = "Vendor-wise Summary  (All Months Combined)"
        ws.cell(vendor_row, 2).font  = Font(name="Arial", bold=True, size=13, color="1F4E79")
        ws.row_dimensions[vendor_row].height = 22

        # Sub-title / hint
        ws.cell(vendor_row+1, 2).value = "PB = Purchase Book  |  GST = GST Portal  |  Diff = PB − GST  (red = gap exists)"
        ws.cell(vendor_row+1, 2).font  = Font(name="Arial", italic=True, size=9, color="595959")
        ...
        for c, h in enumerate(headers, 2):
            ...
            cell.alignment = Alignment(horizontal="center", vertical="center")
            cell.border    = THIN
            if h in pb_set:    cell.fill = hdr_fills["pb"]
            elif h in gst_set: cell.fill = hdr_fills["gst"]
            elif h in diff_set:cell.fill = hdr_fills["diff"]
            else:              cell.fill = hdr_fills["key"]
            if h not in {"GSTIN","Vendor Name (PB)","Vendor Name (GST)"}:
                money_ci.add(c)
        ...
        for r, (_, row) in enumerate(vendor_df.iterrows(), hdr_row+1):
            for c, h in enumerate(headers, 2):
                val  = row[h]
                cell = ws.cell(r, c, val if pd.notna(val) else "")
                cell.border = THIN
        ...
        vtr = hdr_row + 1 + len(vendor_df)
        ws.cell(vtr,2).value = "TOTAL"
        ws.cell(vtr,2).font  = Font(name="Arial",bold=True,size=10)
        ws.cell(vtr,2).fill  = YELLOW_FILL
        for c, h in enumerate(headers, 2):
            if c in money_ci:
                cl = get_column_letter(c); tc = ws.cell(vtr, c)
                tc.value         = f"=SUM({cl}{hdr_row+1}:{cl}{vtr-1})"
                tc.font          = Font(name="Arial", bold=True, size=10)
                tc.number_format = "#,##0.00"
                tc.fill          = YELLOW_FILL
                tc.alignment     = Alignment(horizontal="right")

    # Column widths — vendor table has wide columns, set generously
    col_widths = {1:2, 2:22, 3:14, 4:26, 5:26, 6:14, 7:14, 8:14, 9:14,
                  10:14, 11:14, 12:14, 13:14, 14:16, 15:16, 16:16, 17:16}
...
def _write_flat_summary(ws, matched, mismatched, miss_gst, miss_pb,
                        vendor_df, all_dups):
    """Summary sheet for flat (non-month-split) reconciliation."""
    ...
    for r, (label, df, fhex, at_risk) in enumerate(rows_data, 6):
        ...
        for c, v in enumerate(vals, 2):
            cell = ws.cell(r, c, v)
            cell.font      = Font(name="Arial", size=10,
                                  bold=(at_risk and isinstance(v,float) and v>0))
            cell.fill      = fill
            cell.border    = THIN
            cell.alignment = Alignment(
                horizontal="right" if isinstance(v,(int,float)) else "left",
...
#     # Duplicates note
#     if not all_dups.empty:
#         ws.cell(13, 2).value = f"🗑️  Duplicate Rows Removed: {len(all_dups)}  (see 'Duplicates_Removed' sheet)"
#         ws.cell(13, 2).font  = Font(name="Arial", bold=True, size=10, color="595959")

    # ── Vendor-wise table below ───────────────────────────────────────────────
    if not vendor_df.empty:
        vr = 16   # start row for vendor section
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 11 — FULL PIPELINE  (flat — all files combined, no month split)
# ─────────────────────────────────────────────────────────────────────────────

def run_reconciliation(gst_files: list[str], pb_files: list[str],
                       out_folder: str, log, set_progress) -> str:
    """
    Flat pipeline — no month splitting:
      1. Stack ALL GST files into one DataFrame
      2. Stack ALL PB  files into one DataFrame
      3. Deduplicate, validate, aggregate, reconcile
      4. Export one flat Excel report
    """
    # all_dups = []   # DUPLICATES DISABLED
    set_progress(8)

    # ── Step 1: Load all GST files ────────────────────────────────────────────
    ...
    # ── Step 2: Load all PB files ─────────────────────────────────────────────
    log("\n[2] Loading Purchase Book files …")
    pb_frames = []
    for fp in pb_files:
        log(f"  Reading: {Path(fp).name}")
        pb_frames.append(load_pb_file(fp, log))
    pb_raw = pd.concat(pb_frames, ignore_index=True) if pb_frames else pd.DataFrame()
    log(f"  PB total: {len(pb_raw):,} rows from {len(pb_files)} file(s)")
    set_progress(45)
    ...
    # ── Step 4: Duplicate detection (temporarily disabled) ───────────────────
    # log("\n[3] Checking for duplicates …")
    # pb_clean,  pb_dups  = detect_and_remove_duplicates(pb_raw,  "PB",  log)
    # gst_clean, gst_dups = detect_and_remove_duplicates(gst_raw, "GST", log)
    # if not pb_dups.empty:
    #     pb_dups["Source"] = "Purchase Book"; all_dups.append(pb_dups)
    # if not gst_dups.empty:
    #     gst_dups["Source"] = "GST Portal";   all_dups.append(gst_dups)
    pb_clean  = pb_raw.copy()
    gst_clean = gst_raw.copy()
    set_progress(58)

    # ── Step 5: GSTIN validation ──────────────────────────────────────────────
    log("\n[4] Validating GSTINs …")
    ...
    # ── Step 8: Export ────────────────────────────────────────────────────────
    log("\n[7] Exporting report …")
    # dups_df = pd.concat(all_dups, ignore_index=True) if all_dups else pd.DataFrame()
    dups_df = pd.DataFrame()   # DUPLICATES DISABLED
    path = export_report(matched, mismatched, miss_gst, miss_pb,
                         pb_agg, gst_agg, dups_df, out_folder, log)
    set_progress(100)
    return path
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.gst_files   = []   # list of selected GST file paths
        self.pb_files    = []   # list of selected PB  file paths
    ...
    def _build_ui(self):
        ...
        # ── GST Files picker ──────────────────────────────────────────────────
        self._multi_file_row(
            left,
            "📂  GST Portal Files  (select one or multiple)",
            "You can select April.xlsx + May.xlsx + June.xlsx at once",
            "gst"
        )
        ...
        self._multi_file_row(
            left,
            "📂  Purchase Book Files  (select one or multiple)",
            "Select all months at once",
            "pb"
        )

        # ── Output folder ─────────────────────────────────────────────────────
        out_inner = self._card(left, "💾  Output Folder")
        out_row   = tk.Frame(out_inner, bg=C["card"])
    ...
    def _multi_file_row(self, parent, title, hint, kind):
        """A file-picker card that shows selected filenames as a list."""
        inner = self._card(parent, title)
        tk.Label(inner, text=hint, bg=C["card"],
                 fg=C["subtext"], font=("Arial",8)).pack(anchor="w")

        # Listbox to show selected files
        lb_frame = tk.Frame(inner, bg=C["card"])
        lb_frame.pack(fill="x", pady=(4,0))

        lb = tk.Listbox(lb_frame, height=3, font=("Arial",8),
                        relief="solid", bd=1,
                        selectmode="multiple",
                        bg="#F8FAFC", fg=C["text"],
        ...
        def browse():
            ...
            if paths:
                if kind == "gst":
                    self.gst_files = list(paths)
                else:
                    self.pb_files  = list(paths)
                lb.delete(0, "end")
                for p in paths:
    ...
    def _set_status(self, msg, colour=None):
        self.after(0, lambda: self.status_lbl.configure(
            text=msg, fg=colour or C["subtext"]))

    def _set_progress(self, val):
        self.after(0, lambda: self.progress.set(val))

    def _show_badges(self, month_results):
        def _do():
            for w in self.badge_frame.winfo_children(): w.destroy()
            total_m = sum(len(r[0]) for r in month_results.values())
            total_x = sum(len(r[1]) for r in month_results.values())
            total_g = sum(len(r[2]) for r in month_results.values())
            total_p = sum(len(r[3]) for r in month_results.values())
            data = [
    ...
    def _start(self):
        if not self.gst_files:
            messagebox.showwarning("Missing Files","Please select at least one GST Portal file.")
            return
        if not self.pb_files:
            messagebox.showwarning("Missing Files","Please select at least one Purchase Book file.")
            return
    ...
    def _pipeline(self, gst_files, pb_files, out_folder):
        try:
            ...
            # Store month_results for badges
            month_results_holder = {}

            def patched_export(month_results, all_dups, out_folder, log):
                month_results_holder.update(month_results)
                return export_report(month_results, all_dups, out_folder, log)

            path = run_reconciliation(
                gst_files, pb_files, out_folder,
                self._log, self._set_progress
            )

            # Re-read month results from log for badges (simpler approach)
            # Just show total from the output file
            ...
            self.after(0, lambda: messagebox.showinfo(
                ...
                f"Files saved: {path}\n\n"
                f"  ✅ Matched (total)     : {total_m:,}\n"
                f"  🔴 Mismatched (total)  : {total_x:,}\n"
                f"  ⚠️  Miss. GST (total)   : {total_g:,}\n"
                f"  ⚠️  Miss. PB (total)    : {total_p:,}"
            ))
